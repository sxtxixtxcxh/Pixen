//
//  PXDefaults.m
//  Pixen
//
//  Copyright 2011-2012 Pixen Project. All rights reserved.
//

#import "PXDefaults.h"

NSString *PXGridShouldDrawKey = @"PXGridShouldDraw";
NSString *PXGridUnitWidthKey = @"PXGridUnitWidth";
NSString *PXGridUnitHeightKey = @"PXGridUnitHeight";
NSString *PXGridColorDataKey = @"PXGridColorData";

NSString *PXCrosshairColorKey = @"PXCrosshairColor";
NSString *PXCrosshairEnabledKey = @"PXCrosshairEnabled";

NSString *PXCaresAboutPressureKey = @"PXCaresAboutPressure";

NSString *PXCanvasDefaultMainBackgroundKey = @"PXDefaultCanvasMainBackground";
NSString *PXCanvasDefaultAlternateBackgroundKey = @"PXDefaultCanvasAlternateBackground";
NSString *PXPreviewDefaultMainBackgroundKey = @"PXDefaultPreviewMainBackground";
NSString *PXPreviewDefaultAlternateBackgroundKey = @"PXDefaultPreviewAlternateBackground";

NSString *PXDefaultNewDocumentWidth = @"PXDefaultNewDocumentWidth";
NSString *PXDefaultNewDocumentHeight = @"PXDefaultNewDocumentHeight";
NSString *PXDefaultNewDocumentBackgroundColor = @"PXDefaultNewDocumentBackgroundColor";

NSString *PXLeftToolPropertiesFrameKey = @"PXLeftToolPropertiesFrame";
NSString *PXRightToolPropertiesFrameKey = @"PXRightToolPropertiesFrame";

NSString *PXPreviewWindowIsOpenKey = @"PXPreviewWindowIsOpen";

NSString *PXSelectedScaleAlgorithmKey = @"PXSelectedScaleAlgorithm";

NSString *PXPreviousCelOverlayEnabledKey = @"PXPreviousCelOverlayEnabled";

NSString *PXVersionKey = @"PXVersion";
NSString *PXHasRunBeforeKey = @"PXHasRunBefore";

NSString *PXRightToolSwitcherIsOpenKey = @"PXRightToolSwitcherIsOpen";

NSString *PXColorPickerPaletteViewSizeKey = @"PXColorPickerPaletteViewSize";
NSString *PXPalettePanelsKey = @"PXPalettePanels";
