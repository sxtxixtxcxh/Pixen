...
-(void)	awakeFromNib
{
	...
	if ([box isKindOfClass:[NSBox class]])
	{
		...
		NSRect contentFrame = [[[windowController window] contentView] frame];
		NSRect windowFrame = [[windowController window] frame];
		float yOffset = NSHeight(contentFrame) - NSHeight(boxFrame);
		[[windowController window] setContentSize:NSMakeSize(NSWidth(contentFrame), NSHeight(boxFrame))];
		[[windowController window] setFrameOrigin:NSMakePoint(NSMinX(windowFrame), NSMinY(windowFrame) + yOffset)];
		[box setFrameOrigin:NSMakePoint(NSMinX(boxFrame), 0)];
	}
}
...
-(IBAction)	changePanes: (id)sender
{
	...
	{
		NSRect windowFrame = [[windowController window] frame];
		float sizeDifference = NSHeight([box frame]) - NSHeight([[[[[tabView tabViewItemAtIndex:[tabView indexOfTabViewItem:[tabView selectedTabViewItem]]] view] subviews] objectAtIndex:0] frame]);
		[[windowController window] setFrame:NSMakeRect(NSMinX(windowFrame),
													   NSMinY(windowFrame) - sizeDifference,
													   NSWidth(windowFrame),
													   NSHeight(windowFrame) + sizeDifference)
									display:YES
									animate:YES];
