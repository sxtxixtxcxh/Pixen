...
- (void)deleteKeyPressedInTableView:tableView
{
	if([self isValidDelegateForSelector:_cmd])
	{
		[delegate deleteKeyPressedInTableView:tableView];
	}
}
...
- (NSView *) tableView:(NSTableView *) tableView viewForRow:(NSInteger) row
{
	...
	{
		return [(id<SubviewTableViewControllerDataSourceProtocol>)[self delegate] tableView: tableView 
																																						 viewForRow: row];
	}
	return nil;
}
...
- (void) tableView:(NSTableView *) tableView willDisplayCell:(id) cell forTableColumn:(NSTableColumn *) tableColumn row:(NSInteger) row
{
	if (tableColumn == subviewTableColumn)
	{
		...
		if ([self isValidDelegateForSelector: @selector(tableView:viewForRow:)])
		{
			NSView *sv = [(id<SubviewTableViewControllerDataSourceProtocol>)[self delegate] tableView: tableView 
																																										 viewForRow: row];
			[(SubviewTableViewCell *)cell addSubview:sv];
		}
	}
	else
	{
...
- (NSInteger) numberOfRowsInTableView:(NSTableView *) tableView
{
	...
	if ([self isValidDelegateForSelector: _cmd])
	{
		count = [(id<NSTableViewDataSource>)[self delegate] numberOfRowsInTableView: tableView];
	}
						 ...
						 row:(NSInteger) row
{
	...
	{
		obj = [[self delegate] tableView: tableView 
...
- (void)dragFailedToTableView:tableView
{
	if ([self isValidDelegateForSelector: _cmd])
	{
		return [[self delegate] dragFailedToTableView:tableView];
	}
}
